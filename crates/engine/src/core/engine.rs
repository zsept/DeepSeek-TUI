...
use deepseek_config::{ApiProvider, Config, DEFAULT_MAX_CONCURRENT_AGENTS, DEFAULT_TEXT_MODEL};
...
use deepseek_config::features::{Feature, Features};
...
use deepseek_capacity::capacity::{
...
use deepseek_capacity::capacity_memory::{
...
use deepseek_capacity::coherence::{CoherenceSignal, CoherenceState, next_coherence_state};
...
pub struct EngineConfig {
    ...
    pub agent_role_configs: std::collections::HashMap<String, deepseek_config::AgentRoleConfig>,
    ...
    pub network_policy: Option<deepseek_base::network_policy::NetworkPolicyDecider>,
    ...
    pub vision_config: Option<deepseek_config::VisionModelConfig>,
    ...
    pub workshop: Option<deepseek_config::WorkshopConfig>,
    /// Which search backend `web_search` should use. Default: Bing.
    pub search_provider: deepseek_config::SearchProvider,
...
impl Default for EngineConfig {
    fn default() -> Self {
        Self {
            ...
            workshop: None,
            search_provider: deepseek_config::SearchProvider::default(),
            search_api_key: None,
        }
    }
}
...
impl Engine {
    ...
    fn env_only_api_key_recovery_hint(api_config: &Config) -> Option<String> {
        if !deepseek_config::active_provider_uses_env_only_api_key(api_config) {
            return None;
        }
    ...
    pub fn new(config: EngineConfig, api_config: &Config) -> (Self, EngineHandle) {
        ...
        let user_memory_block =
            deepseek_shared::session::memory::compose_block(config.memory_enabled, &config.memory_path);
        let system_prompt =
        ...
        while let Some(op) = self.rx_op.recv().await {
            match op {
                ...
                Op::SyncSession {
                    ...
                    let ctx = deepseek_context::project_context::load_project_context_with_parents(&workspace);
    ...
    async fn handle_send_message(
        ...
        trust_mode: bool,
        auto_approve: bool,
        approval_mode: deepseek_base::mode_types::ApprovalMode,
        ...
        if self.config.snapshots_enabled {
            ...
            deepseek_support::utils::spawn_blocking_supervised("post-turn-snapshot", move || {
    ...
    fn refresh_system_prompt(&mut self, mode: AppMode) {
        let user_memory_block =
            deepseek_shared::session::memory::compose_block(self.config.memory_enabled, &self.config.memory_path);
