...
use crate::config::features::{Feature, render_feature_table};
use crate::core::llm_client::LlmClient;
use deepseek_mcp::{McpConfig, McpConfigEntry, McpPool};
use deepseek_models::{ContentBlock, Message, MessageRequest, SystemPrompt};
use crate::session::manager::{SessionManager, create_saved_session, truncate_id};

use crate::summarize::{summarize_tool_args, summarize_tool_output};
#[cfg(windows)]
...
fn spawn_signal_cleanup_task() {
    tokio::spawn(async {
        ...
        if !CLEANED_UP.swap(true, std::sync::atomic::Ordering::SeqCst) {
            // Terminal restore handled by tui crate
            #[cfg(unix)]
            unsafe {
                let _ = libc::dup2(2, 1);
                let _ = libc::dup2(2, 0);
...
enum ExecpolicySubcommand {
    ...
    Check(deepseek_execpolicy::ExecPolicyCheckCommand),
}
    ...
    if let Some(command) = cli.command.clone() {
        return match command {
            ...
            Commands::Serve(args) => {
                ...
                if args.mcp {
                    anyhow::bail!("MCP server mode not available in this build")
                } else if args.http {
                    let config = load_config_from_cli(&cli)?;
                    let cors_origins = resolve_cors_origins(&config, &args.cors_origin);
                    crate::runtime::api::run_http_server(
                        config,
                        workspace,
                        crate::runtime::api::RuntimeApiOptions {
                            host: args.host,
                            port: args.port,
...
fn run_setup(config: &Config, workspace: &Path, args: SetupArgs) -> Result<()> {
    ...
    let sandbox = deepseek_sandbox::get_platform_sandbox();
...
fn skills_count_for(dir: &Path) -> usize {
    ...
    deepseek_skills::SkillRegistry::discover(dir).len()
}
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    let agents_global_skills_dir = deepseek_skills::agents_global_skills_dir();
    ...
    if let Some(stash_path) = stash_path {
        let stash_count = crate::session::stash::load_stash().len();
    ...
    if crate::config::settings::detected_legacy_windows_console_host() {
        println!(
    ...
    let report = json!({
        ...
        "storage": {
            ...
            "stash": {
                ...
                "count": crate::session::stash::load_stash().len(),
            },
        },
        "sandbox": match deepseek_sandbox::get_platform_sandbox() {
...
async fn test_api_connectivity(config: &Config) -> Result<()> {
    use crate::core::client::DeepSeekClient;
    use deepseek_models::{ContentBlock, Message, MessageRequest};
...
fn list_sessions(limit: usize, search: Option<String>) -> Result<()> {
    use crate::palette;
    use colored::Colorize;
    use crate::session::manager::{SessionManager, format_session_line};
...
fn doctor_check_mcp_server(server: &McpConfigEntry) -> McpServerDoctorStatus {
...
fn run_sandbox_command(args: SandboxArgs) -> Result<()> {
    use deepseek_sandbox::{CommandSpec, SandboxManager};
...
fn parse_sandbox_policy(
    ...
    exclude_tmpdir: bool,
    exclude_slash_tmp: bool,
) -> Result<deepseek_sandbox::SandboxPolicy> {
...
) -> Option<(crate::session::manager::SavedSession, std::time::Duration)> {
...
fn recover_interrupted_checkpoint_for_resume(launch_workspace: &Path) -> Option<String> {
    let manager = crate::session::manager::SessionManager::default_location().ok()?;
    ...
    let workspace_matches =
        crate::session::manager::workspace_scope_matches(&session_workspace, launch_workspace);
...
fn preserve_interrupted_checkpoint_for_explicit_resume(launch_workspace: &Path) {
    let Some(manager) = crate::session::manager::SessionManager::default_location().ok() else {
        return;
    };
    ...
    let age_str = checkpoint_age_label(age);
    if crate::session::manager::workspace_scope_matches(&session_workspace, launch_workspace) {
        eprintln!(
...
async fn run_interactive(
    ...
    let use_bracketed_paste = crate::config::settings::Settings::load()
    ...
    let skills_dir = config.skills_dir();
    if let Err(e) = deepseek_skills::install_system_skills(&skills_dir) {
    ...
    if snapshots.enabled {
        crate::session::manager::prune_workspace_snapshots(&workspace, snapshots.max_age());
    }
...
struct CliAutoRoute {
    model: String,
    reasoning_effort: Option<crate::mode_types::ReasoningEffort>,
    auto_model: bool,
}
    ...
    if model.trim().eq_ignore_ascii_case("auto") {
        ...
        CliAutoRoute {
            ...
            reasoning_effort: config
                .reasoning_effort()
                .map(crate::mode_types::ReasoningEffort::from_setting),
            auto_model: false,
        }
    }
}
...
fn persist_exec_session(
    ...
    let saved = if let Some(id) = session_id.filter(|id| !id.trim().is_empty()) {
        match manager.load_session(id) {
            Ok(existing) => crate::session::manager::update_session(
                existing,
                messages,
                total_tokens,
                system_prompt.as_ref(),
            ...
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                crate::session::manager::create_saved_session_with_id_and_mode(
                    id.to_string(),
                    messages,
                    model,
                    workspace,
        ...
        crate::session::manager::create_saved_session_with_mode(
            messages,
            model,
            workspace,
            total_tokens,
...
async fn run_exec_agent(
    ...
    use deepseek_models::compaction_threshold_for_model;
    ...
    let engine_config = EngineConfig {
        ...
        compaction,
        cycle: crate::core::cycle_manager::CycleConfig::default(),
        capacity: crate::capacity::capacity::CapacityControllerConfig::from_app_config(config),
        ...
        locale_tag: crate::localization::resolve_locale(
            &crate::config::settings::Settings::load().unwrap_or_default().locale,
        )
        .tag()
        .to_string(),
        ...
        .send(Op::SendMessage {
            ...
            approval_mode: if auto_approve {
                ...
                config
                    .approval_policy
                    .as_deref()
                    .and_then(crate::mode_types::ApprovalMode::from_config_value)
                    .unwrap_or_default()
            },
        })
        .await?;
...
mod doctor_mcp_tests {
    ...
    fn make_server(command: Option<&str>, args: &[&str], url: Option<&str>) -> McpConfigEntry {
        McpConfigEntry {
