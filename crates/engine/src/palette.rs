...
// ── Theme stubs (full impl in crates/tui) ──
#[derive(Debug, Clone)]
pub struct ThemeId {
    name: String,
}
impl ThemeId {
    pub fn new() -> Self { ThemeId { name: "system".to_string() } }
    pub fn from_name(name: &str) -> Option<Self> {
        match name {
            "system" | "dark" | "light" | "grayscale"
            | "catppuccin-mocha" | "tokyo-night" | "dracula" | "gruvbox-dark" => {
                Some(ThemeId { name: name.to_string() })
            }
            "whale" => Some(ThemeId { name: "dark".to_string() }),
            "whale-light" => Some(ThemeId { name: "light".to_string() }),
            _ => None,
        }
    }
    #[must_use]
...
#[derive(Debug, Clone)]
pub struct Theme { pub mode: ThemeMode, pub surface_bg: (u8, u8, u8), }
#[derive(Debug, Clone, Copy)]
pub enum ThemeMode { Dark, Light }

// Use existing functions — don't redefine normalize_theme_name or theme_label_for_mode
