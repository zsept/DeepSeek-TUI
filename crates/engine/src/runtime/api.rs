...
use deepseek_config::{Config, DEFAULT_TEXT_MODEL};
use deepseek_mcp::{McpConfig, McpPool};
...
use deepseek_shared::session::manager::{SavedSession, SessionManager, SessionMetadata, default_sessions_dir};
use deepseek_config::skill_state::SkillStateStore;
...
struct TasksResponse {
    ...
    counts: crate::runtime::task_manager::TaskCounts,
}
        ...
        .map(|msg| {
            let content_blocks: Vec<serde_json::Value> = msg
                ...
                .map(|block| match block {
                    deepseek_models::ContentBlock::Text { text, .. } => {
                        json!({ "type": "text", "text": text })
                    }
                    deepseek_models::ContentBlock::Thinking { thinking, .. } => {
...
fn runtime_event_payload(event: crate::runtime::threads::RuntimeEventRecord) -> serde_json::Value {
    json!({
        "seq": event.seq,
...
fn map_compat_stream_event(event: &crate::runtime::threads::RuntimeEventRecord) -> Option<SseEvent> {
...
mod tests {
    ...
    use crate::runtime::threads::RuntimeEventRecord;
    ...
    impl crate::runtime::task_manager::TaskExecutor for MockExecutor {
        async fn execute(
            &self,
            _task: crate::runtime::task_manager::ExecutionTask,
            events: mpsc::UnboundedSender<crate::runtime::task_manager::TaskExecutionEvent>,
            cancel: tokio_util::sync::CancellationToken,
        ) -> crate::runtime::task_manager::TaskExecutionResult {
            let _ = events.send(crate::runtime::task_manager::TaskExecutionEvent::Status {
                message: "started".to_string(),
            });
            sleep(Duration::from_millis(100)).await;
            if cancel.is_cancelled() {
                return crate::runtime::task_manager::TaskExecutionResult {
                    status: crate::runtime::task_manager::TaskStatus::Canceled,
                    result_text: None,
                    error: None,
                };
            }
            crate::runtime::task_manager::TaskExecutionResult {
                status: crate::runtime::task_manager::TaskStatus::Completed,
    ...
    > {
        ...
        config.capacity = Some(deepseek_config::CapacityConfig {
            enabled: Some(false),
            low_risk_max: None,
