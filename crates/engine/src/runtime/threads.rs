...
use deepseek_models::{ContentBlock, Message, SystemPrompt, Usage, compaction_threshold_for_model};
...
pub struct RuntimeThreadManager {
    ...
    task_manager: Arc<StdMutex<Option<crate::runtime::task_manager::SharedTaskManager>>>,
    automations: Arc<StdMutex<Option<crate::runtime::automation::SharedAutomationManager>>>,
...
impl RuntimeThreadManager {
    ...
    pub fn attach_task_manager(&self, task_manager: crate::runtime::task_manager::SharedTaskManager) {
    ...
    pub fn attach_automation_manager(
        &self,
        automations: crate::runtime::automation::SharedAutomationManager,
    ) {
    ...
    pub async fn aggregate_usage(
        ...
        for thread in self.store.list_threads()? {
            ...
            for turn in turns {
                ...
                let cost = deepseek_support::pricing::calculate_turn_cost_from_usage(&thread.model, usage)
                    .unwrap_or(0.0);
    ...
    pub async fn seed_thread_from_messages(
        ...
        for (user_text, assistant_text) in pending_pairs {
            ...
            let summary = deepseek_support::utils::truncate_with_ellipsis(&user_text, SUMMARY_LIMIT, "...");
    ...
    pub async fn start_turn(&self, thread_id: &str, req: StartTurnRequest) -> Result<TurnRecord> {
        ...
        let (model, reasoning_effort) = if auto_model {
            let default_model = self.config.default_model();
            let selection = deepseek_shared::auto_route::resolve_auto_route_with_flash(
                &self.config,
                &default_model,
                &prompt,
            )
            .await;
            match selection {
                Some(route) => (route.model, route.reasoning_effort),
                None => (requested_model, None),
            }
        } else {
        ...
        engine
            .send(Op::SendMessage {
                ...
                approval_mode: if auto_approve {
                    deepseek_base::mode_types::ApprovalMode::Auto
                } else {
                    deepseek_base::mode_types::ApprovalMode::Suggest
                },
            })
            .await
    ...
    async fn ensure_engine_loaded(&self, thread: &ThreadRecord) -> Result<EngineHandle> {
        ...
        let network_policy = self.config.network.clone().map(|toml_cfg| {
            deepseek_base::network_policy::NetworkPolicyDecider::with_default_audit(toml_cfg.into_runtime())
        });
        let lsp_config = self
            .config
            .lsp
            .clone()
            .map(deepseek_config::LspConfigToml::into_runtime);
        let engine_cfg = EngineConfig {
            ...
            agent_role_configs: self.config.agent_role_configs(crate::tools::agent::builtin_role_configs()),
            ...
            capacity: deepseek_capacity::capacity::CapacityControllerConfig::from_app_config(
                &self.config,
            ),
            ...
            runtime_services: crate::tools::spec::RuntimeToolServices {
                ...
                rlm_sessions: crate::core::rlm::session::new_shared_rlm_session_store(),
            },
            ...
            goal_objective: None,
            locale_tag: deepseek_base::localization::resolve_locale(
                &deepseek_config::settings::Settings::load().unwrap_or_default().locale,
            )
            .tag()
            .to_string(),
            ...
            match event {
                ...
                EngineEvent::AgentList { agents } => {
                    ...
                    let interrupted = agents
                        .iter()
                        .filter(|agent| matches!(agent.status, AgentStatus::Interrupted { .. }))
                        .count();
                        ...
                        RuntimeApprovalDecision::RetryWithFullAccess => {
                            let _ = engine
                                .retry_tool_with_policy(
                                    tool_id,
                                    deepseek_sandbox::SandboxPolicy::DangerFullAccess,
                                )
                                .await;
                        }
                        RuntimeApprovalDecision::ApproveTool
