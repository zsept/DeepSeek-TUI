...
mod tests {
    ...
    fn render_available_skills_context_lists_paths_and_usage() {
        ...
        let rendered =
            crate::render_available_skills_context(&tmpdir.path().join("skills"))
    ...
    fn render_available_skills_context_returns_none_when_empty() {
        ...
        assert!(crate::render_available_skills_context(&empty).is_none());

        let missing = tmpdir.path().join("does-not-exist");
        assert!(crate::render_available_skills_context(&missing).is_none());
    }
