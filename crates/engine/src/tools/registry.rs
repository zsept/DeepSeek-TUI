...
impl ToolRegistry {
    ...
    pub fn to_api_tools_with_cache(&self, enable_cache: bool) -> Vec<Tool> {
        ...
        if enable_cache && let Some(last) = tools.last_mut() {
            last.cache_control = Some(deepseek_models::CacheControl {
...
impl ToolRegistryBuilder {
    ...
    pub fn with_pandoc_tools(self) -> Self {
        if deepseek_support::dependencies::resolve_pandoc().is_some() {
    ...
    pub fn with_image_ocr_tools(self) -> Self {
        if deepseek_support::dependencies::resolve_tesseract().is_some() {
    ...
    #[must_use]
    pub fn with_vision_tools(self, config: deepseek_config::VisionModelConfig) -> Self {
    ...
    pub fn with_mcp_tools(
        mut self,
        mcp_pool: std::sync::Arc<tokio::sync::Mutex<deepseek_mcp::McpPool>>,
    ) -> Self {
...
struct McpToolAdapter {
    ...
    pool: std::sync::Arc<tokio::sync::Mutex<deepseek_mcp::McpPool>>,
}
