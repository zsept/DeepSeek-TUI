...
fn normalize_settings_theme(value: &str) -> String {
    ...
    if trimmed.starts_with("file:") && trimmed.len() > "file:".len() {
        ...
        let path = std::path::PathBuf::from("/dev/null")
            .join(name)
            .with_extension("toml");
        if path.exists() || std::path::PathBuf::from("/dev/null").join(name).exists() {
    ...
    let path = std::path::PathBuf::from("/dev/null")
        .join(trimmed)
        .with_extension("toml");
    if path.exists() || std::path::PathBuf::from("/dev/null").join(trimmed).exists() {
...
fn normalize_theme_setting(value: &str) -> Result<String> {
    ...
    if let Some(file_name) = trimmed.strip_prefix("file:") {
        ...
        if !path.exists() {
            ...
            let bare = std::path::PathBuf::from("/dev/null").join(file_name);
            if !bare.exists() {
                anyhow::bail!(
    ...
    let bare = std::path::PathBuf::from("/dev/null").join(trimmed);
    if bare.exists() {
        return Ok(format!("file:{trimmed}"));
    }
