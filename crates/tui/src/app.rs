...
use crate::files::file_mention::ContextReference;
use crate::render::history::{HistoryCell, TranscriptRenderOptions};
use crate::input::paste_burst::{FlushResult, PasteBurst};
use crate::state::scrolling::{MouseScrollState, TranscriptLineMeta, TranscriptScroll};
use crate::state::selection::{SelectionAutoscroll, TranscriptSelection};
use crate::ui::streaming::StreamingState;
use crate::render::transcript::TranscriptViewCache;
use crate::ui::views::ViewStack;
...
pub(crate) fn resolve_skills_dir(
    ...
    if config.skills_dir.is_none()
        && let Some(global_agents) = deepseek_skills::agents_global_skills_dir()
...
impl ReasoningEffort {
    ...
    pub fn to_engine(self) -> deepseek_shared::mode_types::ReasoningEffort {
        match self {
            Self::Off => deepseek_shared::mode_types::ReasoningEffort::Off,
            Self::Low => deepseek_shared::mode_types::ReasoningEffort::Low,
            Self::Medium => deepseek_shared::mode_types::ReasoningEffort::Medium,
            Self::High => deepseek_shared::mode_types::ReasoningEffort::High,
            Self::Auto => deepseek_shared::mode_types::ReasoningEffort::Auto,
            Self::Max => deepseek_shared::mode_types::ReasoningEffort::Max,
        }
    }
}
...
impl AppMode {
    ...
    pub fn to_engine(self) -> deepseek_shared::mode_types::AppMode {
        match self {
            AppMode::Limited => deepseek_shared::mode_types::AppMode::Limited,
            AppMode::Yolo => deepseek_shared::mode_types::AppMode::Yolo,
            AppMode::Plan => deepseek_shared::mode_types::AppMode::Plan,
        }
    }
}
...
pub struct App {
    ...
    pub agent_role_configs: std::collections::HashMap<String, deepseek_shared::config::AgentRoleConfig>,
    ...
    pub file_tree: Option<crate::files::file_tree::FileTreeState>,
    #[allow(dead_code)]
    ...
    pub backtrack: crate::state::backtrack::BacktrackState,
    ...
    pub status_items: Vec<deepseek_shared::config::StatusItem>,
    ...
    pub mcp_snapshot: Option<deepseek_mcp::McpManagerSnapshot>,
...
impl App {
    ...
    pub fn new(options: TuiOptions, config: &Config) -> Self {
        ...
        let api_key_env_only =
            deepseek_shared::config::active_provider_uses_env_only_api_key(&effective_auth_config);
        ...
        let needs_workspace_trust =
            initial_mode != AppMode::Yolo && crate::onboarding::needs_trust(&workspace);
        ...
        Self {
            ...
            backtrack: crate::state::backtrack::BacktrackState::new(),
            ...
            status_items: config
                ...
                .unwrap_or_else(deepseek_shared::config::StatusItem::default_footer),
            project_doc: None,
            plan_state,
            ...
            mcp_configured_count: deepseek_mcp::load_config(&mcp_config_path)
    ...
    fn discover_cached_skills(workspace: &std::path::Path, extra_dirs: &[PathBuf]) -> Vec<(String, String)> {
        deepseek_skills::discover_in_workspace(workspace, extra_dirs)
            .list()
            .iter()
    ...
    pub fn finish_onboarding(&mut self) {
        ...
        if let Err(err) = crate::onboarding::mark_onboarded() {
    ...
    pub fn set_locale_from_onboarding(&mut self, tag: &str) -> anyhow::Result<()> {
        ...
        settings.save()?;
        self.ui_locale = deepseek_shared::localization::resolve_locale(&settings.locale);
        self.needs_redraw = true;
        Ok(())
    }
    ...
    /// for persistence.
    pub fn sync_cost_to_metadata(&self, metadata: &mut deepseek_shared::session::manager::SessionMetadata) {
    ...
    pub fn format_cost_amount(&self, amount: f64) -> String {
        deepseek_shared::pricing::format_cost_amount(amount, self.cost_currency)
    }

    pub fn format_cost_amount_precise(&self, amount: f64) -> String {
        deepseek_shared::pricing::format_cost_amount_precise(amount, self.cost_currency)
    }
    ...
    pub fn composer_attachment_count(&self) -> usize {
        crate::files::file_mention::media_attachment_references(&self.input).len()
    }
    ...
    pub fn remove_selected_composer_attachment(&mut self) -> bool {
        let references = crate::files::file_mention::media_attachment_references(&self.input);
        let Some(index) = self
            .selected_composer_attachment_index()
    ...
    pub fn submit_input(&mut self) -> Option<String> {
        ...
        if !looks_like_slash_command_input(&input) {
            ...
            deepseek_shared::session::history::append_history(&input);
        }
        self.history_index = None;
...
mod tests {
    ...
    fn composer_paste_flushes_pending_burst_and_normalizes_crlf() {
        ...
        assert!(crate::input::paste::handle_paste_burst_key(
            &mut app, &key, now
        ));
        assert!(
            app.input.is_empty(),
