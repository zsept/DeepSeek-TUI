...
fn show_single_setting(app: &App, key: &str) -> CommandResult {
    ...
    fn locale_display(l: deepseek_shared::localization::Locale) -> &'static str {
        match l {
            deepseek_shared::localization::Locale::En => "en",
            deepseek_shared::localization::Locale::ZhHans => "zh-Hans",
            deepseek_shared::localization::Locale::ZhHant => "zh-Hant",
            deepseek_shared::localization::Locale::Ja => "ja",
            deepseek_shared::localization::Locale::PtBr => "pt-BR",
            deepseek_shared::localization::Locale::Es419 => "es-419",
        }
    }
    ...
    let value = match key.as_str() {
        ...
        "cost_currency" | "currency" => Some(
            match app.cost_currency {
                deepseek_shared::pricing::CostCurrency::Usd => "usd",
                deepseek_shared::pricing::CostCurrency::Cny => "cny",
            }
            .to_string(),
        ),
...
pub fn persist_status_items(items: &[deepseek_shared::config::StatusItem]) -> anyhow::Result<PathBuf> {
    use anyhow::Context;
    use std::fs;
...
pub fn set_config_value(app: &mut App, key: &str, value: &str, persist: bool) -> CommandResult {
    ...
    match key.as_str() {
        ...
        "cost_currency" | "currency" => {
            app.cost_currency = deepseek_shared::pricing::CostCurrency::from_setting(&settings.cost_currency)
                .unwrap_or(deepseek_shared::pricing::CostCurrency::Usd);
            app.needs_redraw = true;
        }
...
fn trust_status(workspace: &Path, app: &App, force_paths: bool) -> CommandResult {
    let trust = deepseek_shared::workspace_trust::WorkspaceTrust::load_for(workspace);
...
fn trust_add(workspace: &Path, raw: &str) -> CommandResult {
    ...
    match deepseek_shared::workspace_trust::add(workspace, &path) {
...
fn trust_remove(workspace: &Path, raw: &str) -> CommandResult {
    ...
    let path = PathBuf::from(expand_tilde(raw));
    match deepseek_shared::workspace_trust::remove(workspace, &path) {
...
pub async fn resolve_auto_route_with_flash(
    config: &deepseek_shared::config::Config,
    latest_request: &str,
    recent_context: &str,
...
fn auto_route_from_heuristic(
    ...
    AutoRouteSelection {
        ...
        reasoning_effort: Some(normalize_auto_route_effort(deepseek_shared::auto_reasoning::select(
            false,
            latest_request,
        ))),
...
mod tests {
    ...
    fn config_auto_cost_saving_defaults_to_false() {
        let cfg = deepseek_shared::config::Config::default();
        assert!(!cfg.auto_cost_saving());
    }

    #[test]
    fn config_auto_cost_saving_reads_table() {
        let cfg = deepseek_shared::config::Config {
            auto: Some(deepseek_shared::config::AutoConfig {
    ...
    fn persist_status_items_writes_tui_section_to_config_toml() {
        ...
        let items = vec![
            deepseek_shared::config::StatusItem::Mode,
            deepseek_shared::config::StatusItem::Model,
            deepseek_shared::config::StatusItem::Cost,
        ];
    ...
    fn persist_status_items_preserves_existing_unrelated_keys() {
        ...
        let written = persist_status_items(&[deepseek_shared::config::StatusItem::Mode])
