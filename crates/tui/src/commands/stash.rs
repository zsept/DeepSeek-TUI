...
use deepseek_shared::session::stash;
...
fn list() -> CommandResult {
    let entries = deepseek_shared::session::stash::load_stash();
    if entries.is_empty() {
...
fn clear() -> CommandResult {
    match deepseek_shared::session::stash::clear_stash() {
...
fn pop(app: &mut App) -> CommandResult {
    match deepseek_shared::session::stash::pop_stash() {
        Some(entry) => {
            ...
            let remaining = deepseek_shared::session::stash::load_stash().len();
