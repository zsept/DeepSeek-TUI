...
use crate::files::file_picker::FilePickerRelevance;
use crate::files::file_picker::FilePickerView;

/// Push the `/files` picker onto the view stack, pre-populated with
/// per-session relevance ranks (modified, @-mentioned, tool-touched).
pub(crate) fn open_file_picker(app: &mut App) {
...
pub(crate) fn build_relevance(app: &App) -> FilePickerRelevance {
    ...
    for record in app.session_context_references.iter().rev().take(64) {
        ...
        if reference.source != deepseek_shared::context_ref::ContextReferenceSource::AtMention {
            continue;
        }
        if !matches!(reference.kind, deepseek_shared::context_ref::ContextReferenceKind::File) {
            continue;
        }
...
pub(crate) fn mark_tool_paths_from_text(
    text: &str,
    workspace: &Path,
    seen: &mut HashSet<String>,
    relevance: &mut FilePickerRelevance,
...
pub(crate) fn workspace_file_candidate(raw: &str, workspace: &Path) -> Option<String> {
