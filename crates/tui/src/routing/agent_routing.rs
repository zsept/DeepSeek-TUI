...
pub(crate) fn active_fanout_counts(app: &App) -> Option<(usize, usize)> {
...
pub(crate) fn reconcile_agent_activity_state(app: &mut App) {
...
pub(crate) fn sort_agents_in_place(agents: &mut [AgentResult]) {
    agents.sort_by(|a, b| {
...
pub(crate) fn handle_agent_mailbox(app: &mut App, seq: u64, message: &MailboxMessage) {
    ...
    if let MailboxMessage::TokenUsage { model, usage, .. } = message {
        if app.session.agent_cost_event_seqs.insert(seq)
            && let Some(cost) =
                deepseek_shared::pricing::calculate_turn_cost_estimate_from_usage(model, usage)
        {
...
pub(crate) fn task_mode_label(mode: AppMode) -> &'static str {
    mode.as_setting()
}

pub(crate) fn task_summary_to_panel_entry(summary: TaskSummary) -> TaskPanelEntry {
    TaskPanelEntry {
        id: summary.id,
...
pub(crate) fn format_task_list(tasks: &[TaskSummary]) -> String {
    if tasks.is_empty() {
...
pub(crate) fn open_task_pager(app: &mut App, task: &TaskRecord) {
    let width = app
        .viewport
...
fn format_task_detail(task: &TaskRecord) -> String {
    ...
    lines.push(format!(
        "Workspace: {}",
        deepseek_shared::utils::display_path(&task.workspace)
    ));
