...
use deepseek_mcp::{McpManagerSnapshot, McpServerSnapshot};
use crate::app::App;
use crate::render::history::HistoryCell;
use crate::render::pager::PagerView;

pub(crate) fn format_mcp_manager(snapshot: &McpManagerSnapshot) -> String {
    let mut lines = vec![
        format!("MCP config: {}", snapshot.config_path.as_deref().unwrap_or("(unknown)")),
...
pub(crate) fn open_mcp_manager_pager(app: &mut App, snapshot: &McpManagerSnapshot) {
    let width = app
        .viewport
...
pub(crate) fn add_mcp_message(app: &mut App, content: String) {
...
mod tests {
    use super::*;
    use deepseek_mcp::McpDiscoveredItem;
