...
use deepseek_tui::tools::shell::{ShellJobDetail, ShellJobSnapshot, ShellResult, ShellStatus};
use crate::app::App;
...
pub(crate) fn format_shell_job_list(jobs: &[ShellJobSnapshot]) -> String {
    ...
    for job in jobs {
        ...
        lines.push(format!("  cwd: {}", deepseek_tui::utils::display_path(&job.cwd)));
...
fn format_shell_job_detail(detail: &ShellJobDetail) -> String {
    ...
    let mut lines = vec![
        ...
        format!("Cwd: {}", deepseek_tui::utils::display_path(&job.cwd)),
