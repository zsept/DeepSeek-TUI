...
use deepseek_tui::tools::ReviewOutput;
use deepseek_tui::tools::spec::{ToolError, ToolResult};
...
pub(crate) fn handle_tool_call_started(
    app: &mut App,
    id: &str,
    name: &str,
...
fn accrue_child_token_cost_if_any(app: &mut App, result: &Result<ToolResult, ToolError>) {
    ...
    let prompt_cache_miss_tokens = metadata
        ...
        .and_then(serde_json::Value::as_u64)
        .map(|v| u32::try_from(v).unwrap_or(u32::MAX));
    let usage = deepseek_models::Usage {
    ...
    if let Some(cost) = deepseek_tui::pricing::calculate_turn_cost_estimate_from_usage(model, &usage) {
...
fn record_spillover_artifact_if_any(
    ...
    app.session_artifacts
        .push(deepseek_tui::artifacts::record_tool_output_artifact_with_size(
            session_id,
            id,
            name,
            storage_path,
...
pub(crate) fn handle_tool_call_complete(
    app: &mut App,
    id: &str,
    name: &str,
    result: &Result<ToolResult, ToolError>,
            ...
            HistoryCell::Tool(ToolCell::Exec(exec)) => {
                ...
                if let Ok(tool_result) = result.as_ref() {
                    ...
                    if status != ToolStatus::Running && exec.interaction.is_none() {
                        ...
                        exec.output_summary =
                            Some(crate::render::history::summarize_tool_output(&tool_result.content));
                    }
                    ...
                    && exec.interaction.is_none()
                {
                    ...
                    exec.output_summary =
                        Some(crate::render::history::summarize_tool_output(&err.to_string()));
                }
                app.mark_history_updated();
            }
...
pub(crate) fn exploring_label(name: &str, input: &serde_json::Value) -> String {
...
pub(crate) fn maybe_add_patch_preview(app: &mut App, input: &serde_json::Value) {
