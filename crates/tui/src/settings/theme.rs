...
use deepseek_shared::palette::PaletteMode;
...
impl ThemeId {
    ...
    pub fn from_name(value: &str) -> Option<Self> {
        match deepseek_shared::palette::normalize_theme_name(value)? {
...
macro_rules! try_override {
    ($theme:ident, $custom:expr, $field:ident, $with:ident) => {
        if let Some(ref c) = $custom.$field {
            if let Some(color) = ::deepseek_shared::palette::parse_hex_rgb_color(c) {
...
impl Theme {
    ...
    pub fn to_engine_stub(&self) -> deepseek_shared::palette::Theme {
        use deepseek_shared::palette::ThemeMode;
        deepseek_shared::palette::Theme {
            mode: match self.mode {
                PaletteMode::Dark => ThemeMode::Dark,
                PaletteMode::Light => ThemeMode::Light,
            },
            surface_bg: (0, 0, 0), // minimal stub, actual color not needed
        }
    }
}
...
mod tests {
    ...
    fn custom_theme_file_parses_reasoning_bg() {
        ...
        let custom: crate::settings::theme::CustomThemeFile =
