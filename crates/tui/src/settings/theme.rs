...
use deepseek_palette::PaletteMode;
...
impl ThemeId {
    ...
    pub fn from_name(value: &str) -> Option<Self> {
        match deepseek_palette::normalize_theme_name(value)? {
...
macro_rules! try_override {
    ($theme:ident, $custom:expr, $field:ident, $with:ident) => {
        if let Some(ref c) = $custom.$field {
            if let Some(color) = ::deepseek_palette::parse_hex_rgb_color(c) {
...
impl Theme {
    ...
    pub fn to_engine_stub(&self) -> deepseek_palette::Theme {
        use deepseek_palette::ThemeMode;
        deepseek_palette::Theme {
