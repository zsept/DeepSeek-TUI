...
use deepseek_shared::tools::plan::StepStatus;
...
use crate::render::history::{GenericToolCell, HistoryCell, ToolCell, ToolStatus, summarize_tool_output};
use crate::routing::agent_routing::active_fanout_counts;
...
fn render_context_panel(f: &mut Frame, area: Rect, app: &App) {
    ...
    let window = deepseek_models::context_window_for_model(&app.model).unwrap_or(1_048_576);
    let pct = if window > 0 {
...
mod tests {
    ...
    fn tasks_panel_collapses_repeated_pending_ci_polls() {
        ...
        assert!(
            text.iter()
                .any(|line| line.contains(crate::input::key_shortcuts::tool_details_shortcut_label())),
    ...
    fn navigator_empty_state_says_no_agents() {
        ...
        let lines = agent_panel_lines(&summary, &[], 32, 8, &crate::settings::theme::DARK_THEME);
    ...
    fn agents_panel_running_state_renders_count_role_and_rows() {
        ...
        let text = lines_to_text(&agent_panel_lines(&summary, &rows, 64, 12, &crate::settings::theme::DARK_THEME));
    ...
    fn navigator_uses_fanout_total_when_fanout_has_seeded_slots() {
        ...
        let text = lines_to_text(&agent_panel_lines(&summary, &[], 64, 8, &crate::settings::theme::DARK_THEME));
    ...
    fn navigator_settled_state_says_done() {
        ...
        let text = lines_to_text(&agent_panel_lines(&summary, &[], 32, 8, &crate::settings::theme::DARK_THEME));
    ...
    fn navigator_truncates_long_role_mix_to_content_width() {
        ...
        let lines = agent_panel_lines(&summary, &[], 16, 8, &crate::settings::theme::DARK_THEME);
