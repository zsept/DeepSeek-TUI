...
impl ApprovalMode {
    ...
    pub fn to_engine(self) -> deepseek_shared::mode_types::ApprovalMode {
        match self {
            ApprovalMode::Auto => deepseek_shared::mode_types::ApprovalMode::Auto,
            ApprovalMode::Suggest => deepseek_shared::mode_types::ApprovalMode::Suggest,
            ApprovalMode::Never => deepseek_shared::mode_types::ApprovalMode::Never,
        }
    }
}
    ...
    match category {
        ...
        ToolCategory::Shell => {
            if let Some(cmd) = params.get("command").and_then(Value::as_str) {
                let _ = deepseek_shared::command_safety::analyze_command(cmd);
            }
