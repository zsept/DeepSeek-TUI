...
use deepseek_tui::runtime::automation::{AutomationManager, AutomationSchedulerConfig, spawn_scheduler};
use deepseek_tui::core::client::{DeepSeekClient, build_cache_warmup_request};
use crate::commands;
use deepseek_tui::core::compaction::estimate_input_tokens_conservative;
use deepseek_tui::config::{ApiProvider, Config, DEFAULT_NVIDIA_NIM_BASE_URL};
...
use deepseek_tui::hooks::{HookEvent, HookExecutor};
...
use deepseek_tui::runtime::task_manager::{
...
use crate::input::composer_ui::*;
use crate::context_inspector::build_context_inspector_text;
use crate::state::event_broker::EventBroker;
use crate::files::file_picker_relevance;
use crate::footer_ui::{
...
use crate::render::live_transcript::LiveTranscriptOverlay;
use crate::routing::mcp_routing::{add_mcp_message, open_mcp_manager_pager};
use crate::input::mouse_ui::*;
use crate::settings::notifications;
pub use crate::onboarding;
use crate::render::pager::PagerView;
use crate::state::persistence_actor::{self, PersistRequest};
use crate::routing::plan_prompt::PlanPromptView;
use crate::state::scrolling::TranscriptScroll;
// SelectionAutoscroll unused
use crate::session_picker::SessionPickerView;
use crate::routing::shell_job_routing::{
    add_shell_job_message, format_shell_job_list, format_shell_poll, open_shell_job_pager,
};
pub use crate::streaming_thinking;
use crate::routing::agent_routing::{
...
#[cfg(test)]
use crate::routing::tool_routing::exploring_label;
use crate::routing::tool_routing::{
    handle_tool_call_complete, handle_tool_call_started, maybe_add_patch_preview,
};
use crate::render::ui_text::{history_cell_to_text, line_to_plain, truncate_line_to_width};
use crate::input::user_input::UserInputView;
use crate::views::agent_view_agents;
use crate::input::vim_mode;
...
use crate::input::slash_menu::{
    apply_slash_menu_selection, try_autocomplete_slash_command, visible_slash_menu_entries,
};
use crate::views::{ConfigView, HelpView, ModalKind, ShellControlView, ViewEvent};
use crate::widgets::pending_input_preview::{ContextPreviewItem, PendingInputPreview};
use crate::widgets::{ChatWidget, ComposerWidget, HeaderData, HeaderWidget, Renderable};
...
pub async fn run_tui(config: &Config, options: TuiOptions) -> Result<()> {
    ...
    crate::settings::osc8::set_enabled(
        config
            .tui
            .as_ref()
    ...
    // start the TUI.
    let _tui_log_guard = match deepseek_utils::runtime::init() {
        Ok(guard) => Some(guard),
        Err(err) => {
    ...
    let sync_output_at_init = !deepseek_tui::config::settings::detected_ptyxis_terminal()
        && !deepseek_tui::config::settings::detected_legacy_windows_console_host();
        ...
        && let Ok(manager) = SessionManager::default_location()
    {
        ...
        let load_result: std::io::Result<Option<deepseek_tui::session::manager::SavedSession>> =
            ...
            Ok(Some(saved)) => {
                ...
                if !recovered {
                    app.status_message = Some(format!(
                        "Resumed session: {}",
                        deepseek_tui::session::manager::truncate_id(&saved.metadata.id)
                    ));
                }
            }
            Ok(None) => {
    ...
    let shell_manager = app
        ...
        .shell_manager
        .clone()
        .unwrap_or_else(|| deepseek_tui::tools::shell::new_shared_shell_manager(app.workspace.clone()));
...
fn handle_memory_quick_add(app: &mut App, input: &str, config: &Config) {
    ...
    match deepseek_tui::session::memory::append_entry(&path, input) {
        Ok(()) => {
...
fn build_engine_config(app: &App, config: &Config) -> EngineConfig {
    EngineConfig {
        ...
        capacity: deepseek_tui::capacity::capacity::CapacityControllerConfig::from_app_config(config),
        todos: app.todos.clone(),
        plan_state: app.plan_state.clone(),
        max_spawn_depth: deepseek_tui::tools::agent::DEFAULT_MAX_SPAWN_DEPTH,
...
async fn refresh_active_task_panel(app: &mut App, task_manager: &SharedTaskManager) {
        ...
        && let Ok(mut mgr) = shell_mgr.lock()
    {
        for job in mgr.list_jobs() {
            if !matches!(job.status, deepseek_tui::tools::shell::ShellStatus::Running) {
                continue;
            }
...
async fn run_event_loop(
    ...
    let mut frame_rate_limiter = crate::render::frame_rate_limiter::FrameRateLimiter::default();
    ...
    loop {
            ...
            match event {
                    ...
                    let text = match translated {
                        Ok(text) => {
                            app.status_message = Some(
                                deepseek_tui::localization::tr(
                                    app.ui_locale,
                                    deepseek_tui::localization::MessageId::TranslationComplete,
                                )
                                .to_string(),
                            );
                            text
                        ...
                        Err(err) => {
                            ...
                            app.status_message = Some(format!(
                                ...
                                deepseek_tui::localization::tr(
                                    app.ui_locale,
                                    deepseek_tui::localization::MessageId::TranslationFailed,
                                )
                            ));
                            deepseek_tui::localization::hidden_translation_failed(app.ui_locale)
                                .to_string()
                        }
                    };
                ...
                TranslationEvent::Thinking {
                    ...
                    let text = match translated {
                        Ok(text) => {
                            app.status_message = Some(
                                deepseek_tui::localization::thinking_translation_complete(app.ui_locale)
                                    .to_string(),
                            );
                            text
                        }
                        Err(err) => {
                            ...
                            app.status_message = Some(format!(
                                "{}: {err}",
                                deepseek_tui::localization::thinking_translation_failed(app.ui_locale)
                            ));
                            deepseek_tui::localization::hidden_translation_failed(app.ui_locale)
                                .to_string()
                        }
                ...
                match event {
                    ...
                    EngineEvent::MessageComplete { .. } => {
                        ...
                        if app.translation_enabled
                            ...
                            && crate::translation::needs_translation(&current_streaming_text)
                            && let Some(translation_client) = translation_client.as_ref()
                        {
                            app.status_message = Some(
                                deepseek_tui::localization::tr(
                                    app.ui_locale,
                                    deepseek_tui::localization::MessageId::TranslationInProgress,
                                )
                                .to_string(),
                            );
                            app.is_loading = true;
                            ...
                            tokio::spawn(async move {
                                let translated = crate::translation::translate_text(
                                    &original_text,
                                    &client,
                                    &translation_model,
                                    &target_language,
                    ...
                    EngineEvent::ThinkingComplete { .. } => {
                        if app.translation_enabled {
                            ...
                            if !original_thinking.is_empty()
                                && crate::translation::needs_translation(&original_thinking)
                                && let Some(translation_client) = translation_client.as_ref()
                            {
                                app.status_message = Some(
                                    deepseek_tui::localization::thinking_translation_in_progress(
                                        app.ui_locale,
                                    )
                                    .to_string(),
                                );
                                ...
                                let translation_model = app
                                    ...
                                    .unwrap_or_else(|| app.model.clone());
                                let placeholder =
                                    deepseek_tui::localization::thinking_translation_placeholder(
                                        app.ui_locale,
                                    )
                                    .to_string();
                                let target_language =
                    ...
                    EngineEvent::ToolCallComplete { id, name, result } => {
                        ...
                        let tool_content = match &result {
                            Ok(output) => sanitize_stream_chunk(
                                &deepseek_tui::core::engine::compact_tool_result_for_context(
                    ...
                    EngineEvent::TurnComplete {
                        ...
                        if !matches!(status, deepseek_tui::core::events::TurnOutcomeStatus::Completed)
                        ...
                        if matches!(
                            status,
                            deepseek_tui::core::events::TurnOutcomeStatus::Interrupted
                                | deepseek_tui::core::events::TurnOutcomeStatus::Failed
                        ) {
                        ...
                        app.runtime_turn_status = Some(match status {
                            deepseek_tui::core::events::TurnOutcomeStatus::Completed => {
                                "completed".to_string()
                            }
                            deepseek_tui::core::events::TurnOutcomeStatus::Interrupted => {
                                "interrupted".to_string()
                            }
                            deepseek_tui::core::events::TurnOutcomeStatus::Failed => "failed".to_string(),
                        });
                        if matches!(
                            status,
                            deepseek_tui::core::events::TurnOutcomeStatus::Interrupted
                        ...
                        let turn_cost = deepseek_tui::pricing::calculate_turn_cost_estimate_from_usage(
                            pricing_model,
                            &usage,
                        );
                        ...
                        if status == deepseek_tui::core::events::TurnOutcomeStatus::Completed
                                ...
                                notifications::settings(config)
                        {
                            ...
                            crate::settings::notifications::notify_done(
                                method,
                                in_tmux,
                                &msg,
                                threshold,
                        ...
                        if status == deepseek_tui::core::events::TurnOutcomeStatus::Interrupted
                            && app.submit_pending_steers_after_interrupt
                        {
                            if let Some(merged) = merge_pending_steers(&mut *app) {
                                queued_to_send = Some(merged);
                            }
                        } else if status == deepseek_tui::core::events::TurnOutcomeStatus::Failed
                    ...
                    EngineEvent::ElevationRequired {
                        ...
                        if app.approval_mode == ApprovalMode::Auto {
                            ...
                            let policy = deepseek_sandbox::SandboxPolicy::DangerFullAccess;
        ...
        let pending_bg_cost = deepseek_tui::cost_status::drain();
        ...
        if event::poll(poll_timeout)? {
            ...
            if key_shortcuts::is_file_tree_toggle_shortcut(&key) {
                if let Some(_state) = app.file_tree.as_mut() {
                    ...
                    let state = crate::files::file_tree::FileTreeState::new(&app.workspace);
            ...
            if (is_plain_char || is_enter) && crate::input::paste::handle_paste_burst_key(app, &key, now) {
                continue;
            }
            ...
            let mention_menu_entries =
                crate::files::file_mention::visible_mention_menu_entries(app, MENTION_MENU_LIMIT);
            ...
            if !matches!(key.code, KeyCode::Esc)
                && matches!(
                    app.backtrack.phase,
                    crate::state::backtrack::BacktrackPhase::Primed
                )
            {
                app.backtrack.reset();
            }
            ...
            match key.code {
                ...
                KeyCode::Esc
                    if app.composer.vim_enabled
                        && app.composer.vim_mode != crate::app::VimMode::Normal =>
                {
                    ...
                    match next_escape_action(app, slash_menu_open) {
                        ...
                        EscapeAction::Noop => {
                            ...
                            match app.backtrack.handle_esc(total) {
                                crate::state::backtrack::EscEffect::None => {}
                                crate::state::backtrack::EscEffect::Prime => {
                                    app.status_message =
                                        Some("Press Esc again to backtrack".to_string());
                                    app.needs_redraw = true;
                                }
                                crate::state::backtrack::EscEffect::Cancel => {
                                    app.status_message = Some("Backtrack canceled".to_string());
                                    app.needs_redraw = true;
                                }
                                crate::state::backtrack::EscEffect::OpenOverlay => {
                ...
                KeyCode::Tab => {
                    if mention_menu_open
                        && crate::files::file_mention::apply_mention_menu_selection(
                            app,
                    ...
                    if crate::files::file_mention::try_autocomplete_file_mention(app) {
                        continue;
                    }
                ...
                KeyCode::Char('o') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                    ...
                    match crate::input::external_editor::spawn_editor_for_input(
                        terminal,
                        app.use_alt_screen,
                        app.use_mouse_capture,
                        app.use_bracketed_paste,
                        ...
                        Ok(crate::input::external_editor::EditorOutcome::Edited(new)) => {
                            app.input = new;
                            app.move_cursor_end();
                        ...
                        Ok(crate::input::external_editor::EditorOutcome::Unchanged) => {
                            app.status_message = Some("Editor closed (no changes)".to_string());
                        }
                        Ok(crate::input::external_editor::EditorOutcome::Cancelled) => {
                    ...
                    if key.modifiers == KeyModifiers::CONTROL && !app.input.is_empty() =>
                {
                    ...
                    deepseek_tui::session::stash::push_stash(&app.input);
                    app.clear_input_recoverable();
                    app.push_status_toast(
...
pub(crate) fn apply_engine_error_to_app(
    app: &mut App,
    envelope: deepseek_tui::core::error_taxonomy::ErrorEnvelope,
) {
    ...
    if app
        .hooks
        .has_hooks_for_event(deepseek_tui::hooks::HookEvent::OnError)
    {
        let context = app.base_hook_context().with_error(&message);
        let _ = app.execute_hooks(deepseek_tui::hooks::HookEvent::OnError, &context);
    }
    ...
    if matches!(
        envelope.category,
        deepseek_tui::core::error_taxonomy::ErrorCategory::Authentication
...
fn queued_message_content_for_app(
    ...
    let user_request = crate::files::file_mention::user_request_with_file_mentions(
        &message.display,
        &app.workspace,
        cwd,
    );
...
async fn dispatch_user_message(
    ...
    if app
        .hooks
        .has_hooks_for_event(deepseek_tui::hooks::HookEvent::MessageSubmit)
    {
        let context = app.base_hook_context().with_message(&message.display);
        let _ = app.execute_hooks(deepseek_tui::hooks::HookEvent::MessageSubmit, &context);
    }
    ...
    let references = crate::files::file_mention::context_references_from_input(
        &message.display,
        &app.workspace,
        cwd.clone(),
    );
    ...
    if let Err(err) = engine_handle
        .send(Op::SendMessage {
            ...
            approval_mode: app.approval_mode.to_engine(),
            translation_enabled: app.translation_enabled,
        })
        .await
    {
...
async fn apply_model_and_compaction_update(
    engine_handle: &EngineHandle,
    compaction: deepseek_tui::core::compaction::CompactionConfig,
) {
    let _ = engine_handle
        .send(Op::SetModel {
...
async fn apply_model_picker_choice(
    ...
    engine_handle: &EngineHandle,
    model: String,
    mut effort: crate::app::ReasoningEffort,
    ...
    match deepseek_tui::config::settings::Settings::load() {
        Ok(mut settings) => {
            if model_changed {
...
async fn switch_provider(
    ...
    *engine_handle = spawn_engine(engine_config, config);
    ...
    if let Ok(mut settings) = deepseek_tui::config::settings::Settings::load() {
...
async fn apply_command_result(
    ...
    if let Some(action) = result.action {
        let action = match action {
            crate::commands::CommandAction::App(a) => a,
            _ => return Ok(false),
        };
        match action {
            AppAction::Quit => {
            ...
            AppAction::FetchModels => {
                if deepseek_tui::config::provider_passes_model_through(config.api_provider()) {
                    app.add_message(HistoryCell::System {
                        content: format!(
                ...
                if app.view_stack.top_kind() != Some(ModalKind::ModelPicker) {
                    ...
                    let picker = match fetch_available_models(config).await {
                        Ok(models) if !models.is_empty() => {
                            app.status_message = Some(format!("Found {} model(s)", models.len()));
                            crate::settings::model_picker::ModelPickerView::new_with_models(app, models)
                        }
                        Ok(_) => {
                            app.status_message = Some(format!(
                            ...
                            crate::settings::model_picker::ModelPickerView::new(app)
                        }
                        Err(error) => {
                            app.add_message(HistoryCell::System {
                                content: format!(
            ...
            AppAction::OpenProviderPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::ProviderPicker) {
                    app.view_stack
                        .push(crate::settings::provider_picker::ProviderPickerView::new(
                            app.api_provider,
                            config,
                        ));
                }
            ...
            AppAction::OpenModePicker => {
                if app.view_stack.top_kind() != Some(ModalKind::ModePicker) {
                    app.view_stack
                        .push(crate::views::mode_picker::ModePickerView::new(
                            app.mode,
                        ));
                }
            }
            AppAction::OpenStatusPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::StatusPicker) {
                    app.view_stack
                        .push(crate::views::status_picker::StatusPickerView::new(
                            &app.status_items,
                        ));
                }
            }
            AppAction::OpenFeedbackPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::FeedbackPicker) {
                    app.view_stack
                        .push(crate::settings::feedback_picker::FeedbackPickerView::new());
                }
            }
            AppAction::OpenThemePicker => {
                if app.view_stack.top_kind() != Some(ModalKind::ThemePicker) {
                    ...
                    app.view_stack
                        .push(crate::settings::theme_picker::ThemePickerView::new(original));
                }
            }
...
fn apply_workspace_runtime_state(app: &mut App, config: &Config, workspace: PathBuf) {
    ...
    app.skills_dir = crate::app::resolve_skills_dir(&workspace, &config.skills_dir(), config);
    ...
    let shell_manager = deepseek_tui::tools::shell::new_shared_shell_manager(workspace);
...
async fn handle_mcp_ui_action(
    app: &mut App,
    config: &Config,
    action: crate::app::McpUiAction,
) {
    use deepseek_mcp::{self, McpWriteStatus};
    ...
    let action_result = match action {
        crate::app::McpUiAction::Show => Ok(()),
        crate::app::McpUiAction::Init { force } => {
            changed = true;
            match crate::mcp::init_config(&path, force) {
        ...
        crate::app::McpUiAction::AddStdio {
            name,
            command,
            args,
        } => {
            changed = true;
            crate::mcp::add_server_config(&path, name.clone(), Some(command), None, args)
                .map(|_| message = Some(format!("Added MCP stdio server '{name}'")))
        }
        crate::app::McpUiAction::AddHttp { name, url } => {
            changed = true;
            crate::mcp::add_server_config(&path, name.clone(), None, Some(url), Vec::new())
                .map(|_| message = Some(format!("Added MCP HTTP/SSE server '{name}'")))
        }
        crate::app::McpUiAction::Enable { name } => {
            changed = true;
            crate::mcp::set_server_enabled(&path, &name, true)
                .map(|_| message = Some(format!("Enabled MCP server '{name}'")))
        }
        crate::app::McpUiAction::Disable { name } => {
            changed = true;
            crate::mcp::set_server_enabled(&path, &name, false)
                .map(|_| message = Some(format!("Disabled MCP server '{name}'")))
        }
        crate::app::McpUiAction::Remove { name } => {
            changed = true;
            crate::mcp::remove_server_config(&path, &name)
                .map(|_| message = Some(format!("Removed MCP server '{name}'")))
        }
        crate::app::McpUiAction::Validate | crate::app::McpUiAction::Reload => Ok(()),
    };
    ...
    let snapshot_result = if discover {
        ...
        crate::mcp::manager_snapshot_from_config(&path, app.mcp_restart_required)
    };
...
fn mcp_ui_action_refreshes_discovery(action: &crate::app::McpUiAction) -> bool {
    matches!(
        action,
        crate::app::McpUiAction::Show
            | crate::app::McpUiAction::Validate
            | crate::app::McpUiAction::Reload
    )
}

fn handle_shell_job_action(app: &mut App, action: crate::app::ShellJobAction) {
    ...
    match action {
        ...
        crate::app::ShellJobAction::SendStdin { id, input, close } => {
        ...
        crate::app::ShellJobAction::Cancel { id } => match manager.kill(&id) {
        ...
        crate::app::ShellJobAction::CancelAll => match manager.kill_running() {
            Ok(results) => {
...
async fn execute_command_input(
    ...
    if input.trim().eq_ignore_ascii_case("/logout") {
        ...
        app.api_key_env_only = deepseek_tui::config::active_provider_uses_env_only_api_key(config);
    }
...
fn build_pending_input_preview(app: &App) -> PendingInputPreview {
    ...
    preview.context_items = crate::files::file_mention::pending_context_previews(
        &app.input,
        &app.workspace,
...
fn render(f: &mut Frame, app: &mut App) {
    ...
    // Render header
    {
        let sanitized_context_window = context_usage
            .as_ref()
            .map(|(_, max, _)| *max)
            .or_else(|| deepseek_models::context_window_for_model(&app.model));
        ...
        let provider_label = match app.api_provider {
            deepseek_tui::config::ApiProvider::Deepseek => None,
            deepseek_tui::config::ApiProvider::DeepseekCN => None,
            deepseek_tui::config::ApiProvider::NvidiaNim => Some("NIM"),
            deepseek_tui::config::ApiProvider::Openai => Some("OpenAI"),
            deepseek_tui::config::ApiProvider::Atlascloud => Some("Atlas"),
            deepseek_tui::config::ApiProvider::Openrouter => Some("OR"),
            deepseek_tui::config::ApiProvider::Novita => Some("Novita"),
            deepseek_tui::config::ApiProvider::Fireworks => Some("Fireworks"),
            deepseek_tui::config::ApiProvider::Sglang => Some("SGLang"),
            deepseek_tui::config::ApiProvider::Vllm => Some("vLLM"),
            deepseek_tui::config::ApiProvider::Ollama => Some("Ollama"),
        };
        ...
        .with_status_indicator(crate::widgets::header_status_indicator_frame(
        ...
        let mut chat_area =
            if app.file_tree.is_some() && chunks[1].width >= SIDEBAR_VISIBLE_MIN_WIDTH {
                ...
                if let Some(ref mut state) = app.file_tree {
                    crate::files::file_tree::render_file_tree(f, tree_area, state, &app.theme);
                }
    ...
    for event in events {
        match event {
            ViewEvent::CommandPaletteSelected { action } => match action {
                crate::views::CommandPaletteAction::ExecuteCommand { command } => {
                    if execute_command_input(
                        terminal,
                        app,
                        engine_handle,
                ...
                crate::views::CommandPaletteAction::InsertText { text } => {
                    app.input = text;
                ...
                crate::views::CommandPaletteAction::OpenTextPager { title, content } => {
            ...
            ViewEvent::ElevationDecision {
                ...
                use crate::state::approval::ElevationOption;
                match option {
            ...
            ViewEvent::ModeSelected { mode } => {
                let result = commands::switch_mode(app, mode.to_engine());
                app.add_message(HistoryCell::System { content: result.message.unwrap_or_default() });
            }
...
async fn apply_provider_picker_api_key(
    ...
    use deepseek_tui::config::{ProviderConfig, ProvidersConfig, save_api_key_for};
...
fn jump_to_adjacent_tool_cell(app: &mut App, direction: SearchDirection) -> bool {
    ...
    let current_cell = line_meta
        .get(top)
        .and_then(crate::state::scrolling::TranscriptLineMeta::cell_line)
...
fn history_has_live_motion(history: &[HistoryCell]) -> bool {
    use crate::render::history::AgentCell;
    use crate::widgets::agent_card::AgentLifecycle;
...
fn spillover_pager_section(app: &App, cell_index: usize) -> Option<String> {
    use crate::render::history::{GenericToolCell, HistoryCell, ToolCell};
...
// Re-exports for code that still references the old `crate::ui::*` paths
// (pre-flatten module layout).
pub use crate::app;
pub use crate::context_inspector;
pub use crate::files;
pub use crate::footer_ui;
pub use crate::input;
pub use crate::render;
pub use crate::routing;
pub use crate::session_picker;
