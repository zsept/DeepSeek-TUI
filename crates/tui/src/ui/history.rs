...
use crate::ui::app::TranscriptSpacing;
...
fn render_archived_context(
    ...
    let rendered = crate::ui::markdown_render::render_markdown(&body, content_width, body_style);
...
fn output_rows(output: &str, width: u16) -> Vec<OutputRow> {
    ...
    for line in output.lines() {
        sanitized.clear();
        crate::ui::osc8::strip_ansi_into(line, &mut sanitized);
...
fn render_cycle_boundary(content: &str, width: u16) -> Vec<Line<'static>> {
    ...
    let rendered =
        crate::ui::markdown_render::render_markdown(content, content_width as u16, style);
