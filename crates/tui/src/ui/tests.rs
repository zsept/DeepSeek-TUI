...
use deepseek_tui::core::engine::mock_engine_handle;
...
fn saved_session_with_messages(messages: Vec<Message>) -> SavedSession {
    SavedSession {
        schema_version: 1,
        metadata: deepseek_tui::session::manager::SessionMetadata {
...
async fn model_change_update_syncs_engine_model_before_compaction() {
    ...
    match engine.rx_op.recv().await.expect("set compaction op") {
        deepseek_tui::core::ops::Op::SetCompaction { config } => {
...
fn saved_default_provider_syncs_back_to_runtime_config() {
    ...
    let settings = deepseek_tui::config::settings::Settings {
...
fn make_agent(
    id: &str,
    status: deepseek_tui::tools::agent::AgentStatus,
) -> deepseek_tui::tools::agent::AgentResult {
    deepseek_tui::tools::agent::AgentResult {
        name: id.to_string(),
        agent_id: id.to_string(),
        context_mode: "fresh".to_string(),
        fork_context: false,
        agent_type: deepseek_tui::tools::agent::AgentRole::General,
        assignment: deepseek_tui::tools::agent::AgentAssignment {
...
fn sort_agents_orders_running_before_terminal_statuses() {
    let mut agents = vec![
        make_agent("agent_c", deepseek_tui::tools::agent::AgentStatus::Completed),
        make_agent("agent_a", deepseek_tui::tools::agent::AgentStatus::Running),
        make_agent(
            "agent_b",
            deepseek_tui::tools::agent::AgentStatus::Failed("boom".to_string()),
        ),
    ];
...
fn running_agent_count_unions_cache_and_progress() {
    ...
    app.agent_cache = vec![
        make_agent("agent_a", deepseek_tui::tools::agent::AgentStatus::Running),
        make_agent("agent_b", deepseek_tui::tools::agent::AgentStatus::Completed),
    ];
    app.agent_progress
...
fn subagent_token_usage_updates_live_cost_counter_without_card_change() {
    ...
    handle_agent_mailbox(
        &mut app,
        1,
        &deepseek_tui::tools::agent::MailboxMessage::TokenUsage {
...
fn subagent_token_usage_is_deduped_by_mailbox_sequence() {
    let mut app = create_test_app();
    let usage = deepseek_tui::tools::agent::MailboxMessage::TokenUsage {
        agent_id: "agent-a".to_string(),
        model: "deepseek-v4-flash".to_string(),
        usage: deepseek_models::Usage {
            input_tokens: 10_000,
...
fn footer_coherence_chip_hides_healthy_and_uses_clear_labels() {
    ...
    app.coherence_state = deepseek_tui::capacity::coherence::CoherenceState::Healthy;
    assert!(
    ...
    app.coherence_state = deepseek_tui::capacity::coherence::CoherenceState::GettingCrowded;
    assert!(
        footer_coherence_spans(&app).is_empty(),
    ...
    let cases = [
        (
            deepseek_tui::capacity::coherence::CoherenceState::RefreshingContext,
            "refreshing context",
        ),
        (
            deepseek_tui::capacity::coherence::CoherenceState::VerifyingRecentWork,
            "verifying",
        ),
        (
            deepseek_tui::capacity::coherence::CoherenceState::ResettingPlan,
            "resetting plan",
        ),
    ];
...
fn footer_auxiliary_spans_use_configured_cost_currency() {
    let mut app = create_test_app();
    app.cost_currency = deepseek_tui::pricing::CostCurrency::Cny;
...
async fn dismissed_plan_prompt_leaves_non_numeric_input_for_normal_send_path() {
    ...
    let engine = deepseek_tui::core::engine::mock_engine_handle();
    let config = Config::default();
...
fn ok_result(
    ...
    Ok(deepseek_tui::tools::spec::ToolResult::success(content))
}

#[test]
fn tool_child_usage_metadata_updates_live_cost_counter() {
    let mut app = create_test_app();
    let result = Ok(deepseek_tui::tools::spec::ToolResult::success("ok").with_metadata(
        serde_json::json!({
...
fn spilled_tool_completion_records_session_artifact_metadata() {
    ...
    let result = Ok(
        deepseek_tui::tools::spec::ToolResult::success("checking crate ...").with_metadata(
            serde_json::json!({
    ...
    assert_eq!(artifact.kind, deepseek_tui::artifacts::ArtifactKind::ToolOutput);
    ...
    let manager =
        deepseek_tui::session::manager::SessionManager::new(tmp.path().join("sessions")).expect("manager");
...
fn apply_loaded_session_restores_artifact_registry() {
    ...
    session.artifacts.push(deepseek_tui::artifacts::ArtifactRecord {
...
fn engine_error_finalizes_active_thinking_block() {
    use deepseek_tui::core::error_taxonomy::StreamError;
...
fn recoverable_engine_error_does_not_enter_offline_mode() {
    use deepseek_tui::core::error_taxonomy::{ErrorEnvelope, StreamError};
    let mut app = create_test_app();
...
fn non_recoverable_engine_error_enters_offline_mode() {
    use deepseek_tui::core::error_taxonomy::ErrorEnvelope;
    let mut app = create_test_app();
    assert!(!app.offline_mode);
...
fn render_footer_from_with_default_items_renders_mode_and_model() {
    ...
    let mut app = create_test_app();
    app.session.session_cost = 0.00005;
    let items = deepseek_tui::config::StatusItem::default_footer();
...
fn default_footer_keeps_prefix_stability_opt_in() {
    ...
    assert!(
        !items.contains(&deepseek_tui::config::StatusItem::PrefixStability),
        "prefix stability is a diagnostic chip and should not crowd the default footer"
    );
    assert!(
        items.contains(&deepseek_tui::config::StatusItem::Cache),
...
fn render_footer_from_prefix_stability_item_renders_cache_slot_chip() {
    ...
    let props = render_footer_from(&app, &[deepseek_tui::config::StatusItem::PrefixStability], None);
...
fn render_footer_from_preserves_prefix_then_cache_order() {
    ...
    let props = render_footer_from(
        &app,
        &[
            deepseek_tui::config::StatusItem::PrefixStability,
            deepseek_tui::config::StatusItem::Cache,
        ],
        None,
    );
...
fn render_footer_from_drops_only_unselected_clusters() {
    ...
    let items: Vec<deepseek_tui::config::StatusItem> = deepseek_tui::config::StatusItem::default_footer()
        .into_iter()
        .filter(|item| *item != deepseek_tui::config::StatusItem::Cost)
        .collect();
    let props = render_footer_from(&app, &items, None);
    assert_eq!(props.mode_label, "limited");
    assert!(!props.model.is_empty(), "footer should show a model name");
...
fn render_footer_from_git_branch_item_renders_workspace_branch() {
    ...
    let props = render_footer_from(&app, &[deepseek_tui::config::StatusItem::GitBranch], None);
...
fn notification_settings_tui_always_keeps_configured_method_no_threshold() {
    let config = Config {
        tui: Some(deepseek_tui::config::TuiConfig {
            notification_condition: Some(deepseek_tui::config::NotificationCondition::Always),
            ..Default::default()
        }),
        notifications: Some(deepseek_tui::config::NotificationsConfig {
            method: deepseek_tui::config::NotificationMethod::Bel,
            threshold_secs: 120,
...
fn notification_settings_tui_never_disables_notifications() {
    let config = Config {
        tui: Some(deepseek_tui::config::TuiConfig {
            notification_condition: Some(deepseek_tui::config::NotificationCondition::Never),
            ..Default::default()
        }),
        ..Config::default()
    };
...
fn notification_settings_no_tui_override_uses_notifications_block() {
    let config = Config {
        notifications: Some(deepseek_tui::config::NotificationsConfig {
            method: deepseek_tui::config::NotificationMethod::Osc9,
            threshold_secs: 45,
