...
use crate::ui::auto_router;
use crate::ui::color_compat::ColorCompatBackend;
use crate::ui::command_palette::{
    CommandPaletteView, build_entries as build_command_palette_entries,
};
use crate::ui::composer_ui::*;
use crate::ui::context_inspector::build_context_inspector_text;
use crate::ui::event_broker::EventBroker;
use crate::ui::file_picker_relevance;
...
use crate::ui::live_transcript::LiveTranscriptOverlay;
use crate::ui::mcp_routing::{add_mcp_message, open_mcp_manager_pager};
use crate::ui::mouse_ui::*;
use crate::ui::notifications;
use crate::ui::onboarding;
use crate::ui::pager::PagerView;
use crate::ui::persistence_actor::{self, PersistRequest};
use crate::ui::plan_prompt::PlanPromptView;
use crate::ui::scrolling::TranscriptScroll;
// SelectionAutoscroll unused
use crate::ui::session_picker::SessionPickerView;
use crate::ui::shell_job_routing::{
...
#[cfg(test)]
use crate::ui::tool_routing::exploring_label;
use crate::ui::tool_routing::{
    handle_tool_call_complete, handle_tool_call_started, maybe_add_patch_preview,
};
use crate::ui::ui_text::{history_cell_to_text, line_to_plain, truncate_line_to_width};
use crate::ui::user_input::UserInputView;
use crate::ui::views::agent_view_agents;
use crate::ui::vim_mode;
use crate::ui::workspace_context;
...
async fn run_event_loop(
    ...
    let mut frame_rate_limiter = crate::ui::frame_rate_limiter::FrameRateLimiter::default();
    ...
    loop {
                ...
                match event {
                    ...
                    EngineEvent::MessageComplete { .. } => {
                        ...
                        if app.translation_enabled
                            ...
                            && crate::ui::translation::needs_translation(&current_streaming_text)
                            && let Some(translation_client) = translation_client.as_ref()
                        {
                            ...
                            tokio::spawn(async move {
                                let translated = crate::ui::translation::translate_text(
                                    &original_text,
                                    &client,
                                    &translation_model,
                                    &target_language,
                    ...
                    EngineEvent::ThinkingComplete { .. } => {
                        if app.translation_enabled {
                            ...
                            if !original_thinking.is_empty()
                                && crate::ui::translation::needs_translation(&original_thinking)
                                && let Some(translation_client) = translation_client.as_ref()
                            {
                                app.status_message = Some(
        ...
        if event::poll(poll_timeout)? {
            ...
            let mention_menu_entries =
                crate::ui::file_mention::visible_mention_menu_entries(app, MENTION_MENU_LIMIT);
            ...
            if !matches!(key.code, KeyCode::Esc)
                && matches!(
                    app.backtrack.phase,
                    crate::ui::backtrack::BacktrackPhase::Primed
                )
            {
                app.backtrack.reset();
            }
            ...
            match key.code {
                ...
                KeyCode::Esc
                    if app.composer.vim_enabled
                        && app.composer.vim_mode != crate::ui::app::VimMode::Normal =>
                {
                    ...
                    match next_escape_action(app, slash_menu_open) {
                        ...
                        EscapeAction::Noop => {
                            ...
                            match app.backtrack.handle_esc(total) {
                                crate::ui::backtrack::EscEffect::None => {}
                                crate::ui::backtrack::EscEffect::Prime => {
                                    app.status_message =
                                        Some("Press Esc again to backtrack".to_string());
                                    app.needs_redraw = true;
                                }
                                crate::ui::backtrack::EscEffect::Cancel => {
                                    app.status_message = Some("Backtrack canceled".to_string());
                                    app.needs_redraw = true;
                                }
                                crate::ui::backtrack::EscEffect::OpenOverlay => {
                ...
                KeyCode::Tab => {
                    if mention_menu_open
                        && crate::ui::file_mention::apply_mention_menu_selection(
                            app,
                    ...
                    if crate::ui::file_mention::try_autocomplete_file_mention(app) {
                        continue;
                    }
...
fn queued_message_content_for_app(
    ...
    let user_request = crate::ui::file_mention::user_request_with_file_mentions(
        &message.display,
        &app.workspace,
        cwd,
    );
...
async fn dispatch_user_message(
    ...
    let references = crate::ui::file_mention::context_references_from_input(
        &message.display,
        &app.workspace,
        cwd.clone(),
    );
...
async fn apply_model_picker_choice(
    ...
    engine_handle: &EngineHandle,
    model: String,
    mut effort: crate::ui::app::ReasoningEffort,
    previous_model: String,
    previous_effort: crate::ui::app::ReasoningEffort,
) {
        ...
        match action {
                ...
                if app.view_stack.top_kind() != Some(ModalKind::ModelPicker) {
                    ...
                    let picker = match fetch_available_models(config).await {
                        Ok(models) if !models.is_empty() => {
                            app.status_message = Some(format!("Found {} model(s)", models.len()));
                            crate::ui::model_picker::ModelPickerView::new_with_models(app, models)
                        }
                        Ok(_) => {
                            app.status_message = Some(format!(
                            ...
                            crate::ui::model_picker::ModelPickerView::new(app)
                        }
                        Err(error) => {
                            app.add_message(HistoryCell::System {
                                content: format!(
            ...
            AppAction::OpenProviderPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::ProviderPicker) {
                    app.view_stack
                        .push(crate::ui::provider_picker::ProviderPickerView::new(
                            app.api_provider,
                            config,
                        ));
                }
            ...
            AppAction::OpenModePicker => {
                if app.view_stack.top_kind() != Some(ModalKind::ModePicker) {
                    app.view_stack
                        .push(crate::ui::views::mode_picker::ModePickerView::new(
                            app.mode,
                        ));
                }
            }
            AppAction::OpenStatusPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::StatusPicker) {
                    app.view_stack
                        .push(crate::ui::views::status_picker::StatusPickerView::new(
                            &app.status_items,
                        ));
                }
            }
            AppAction::OpenFeedbackPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::FeedbackPicker) {
                    app.view_stack
                        .push(crate::ui::feedback_picker::FeedbackPickerView::new());
                }
            }
            AppAction::OpenThemePicker => {
                if app.view_stack.top_kind() != Some(ModalKind::ThemePicker) {
                    ...
                    app.view_stack
                        .push(crate::ui::theme_picker::ThemePickerView::new(original));
                }
            }
...
fn apply_workspace_runtime_state(app: &mut App, config: &Config, workspace: PathBuf) {
    ...
    app.skills_dir = crate::ui::app::resolve_skills_dir(&workspace, &config.skills_dir(), config);
...
async fn handle_mcp_ui_action(
    app: &mut App,
    config: &Config,
    action: crate::ui::app::McpUiAction,
) {
    ...
    let action_result = match action {
        crate::ui::app::McpUiAction::Show => Ok(()),
        crate::ui::app::McpUiAction::Init { force } => {
            changed = true;
        ...
        crate::ui::app::McpUiAction::AddStdio {
            name,
            command,
            args,
        } => {
...
fn mcp_ui_action_refreshes_discovery(action: &crate::ui::app::McpUiAction) -> bool {
    matches!(
        action,
        crate::ui::app::McpUiAction::Show
            | crate::ui::app::McpUiAction::Validate
            | crate::ui::app::McpUiAction::Reload
    )
}

fn handle_shell_job_action(app: &mut App, action: crate::ui::app::ShellJobAction) {
    ...
    match action {
        ...
        crate::ui::app::ShellJobAction::SendStdin { id, input, close } => {
        ...
        crate::ui::app::ShellJobAction::Cancel { id } => match manager.kill(&id) {
        ...
        crate::ui::app::ShellJobAction::CancelAll => match manager.kill_running() {
            Ok(results) => {
...
fn build_pending_input_preview(app: &App) -> PendingInputPreview {
    ...
    preview.context_items = crate::ui::file_mention::pending_context_previews(
        &app.input,
        &app.workspace,
...
fn render(f: &mut Frame, app: &mut App) {
    ...
    // Render header
    {
        ...
        .with_status_indicator(crate::ui::widgets::header_status_indicator_frame(
    ...
    for event in events {
        match event {
            ViewEvent::CommandPaletteSelected { action } => match action {
                crate::ui::views::CommandPaletteAction::ExecuteCommand { command } => {
                    if execute_command_input(
                        terminal,
                        app,
                        engine_handle,
                ...
                crate::ui::views::CommandPaletteAction::InsertText { text } => {
                    app.input = text;
                ...
                crate::ui::views::CommandPaletteAction::OpenTextPager { title, content } => {
            ...
            ViewEvent::ElevationDecision {
                ...
                use crate::ui::approval::ElevationOption;
                match option {
...
fn jump_to_adjacent_tool_cell(app: &mut App, direction: SearchDirection) -> bool {
    ...
    let current_cell = line_meta
        .get(top)
        .and_then(crate::ui::scrolling::TranscriptLineMeta::cell_line)
...
fn history_has_live_motion(history: &[HistoryCell]) -> bool {
    use crate::ui::history::AgentCell;
...
fn spillover_pager_section(app: &App, cell_index: usize) -> Option<String> {
    use crate::ui::history::{GenericToolCell, HistoryCell, ToolCell};
