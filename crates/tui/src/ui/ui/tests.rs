...
fn copy_shortcut_accepts_cmd_and_ctrl_shift_only() {
    assert!(crate::ui::key_shortcuts::is_copy_shortcut(&KeyEvent::new(
        KeyCode::Char('c'),
        KeyModifiers::SUPER,
    )));
    assert!(crate::ui::key_shortcuts::is_copy_shortcut(&KeyEvent::new(
    ...
    assert!(!crate::ui::key_shortcuts::is_copy_shortcut(
...
fn file_tree_shortcut_does_not_steal_plain_ctrl_e() {
    assert!(!crate::ui::key_shortcuts::is_file_tree_toggle_shortcut(
...
fn parse_git_status_path_handles_simple_and_renamed_entries() {
    assert_eq!(
        crate::ui::file_picker_relevance::parse_git_status_path(" M crates/tui/src/tui/ui.rs"),
        Some("crates/tui/src/tui/ui.rs".to_string())
    );
    assert_eq!(
        crate::ui::file_picker_relevance::parse_git_status_path(
...
fn workspace_file_candidate_normalizes_absolute_and_line_suffixed_paths() {
    ...
    assert_eq!(
        crate::ui::file_picker_relevance::workspace_file_candidate(&raw, root),
...
fn tool_path_relevance_extracts_paths_from_command_text() {
    ...
    let mut relevance = crate::ui::file_picker::FilePickerRelevance::default();
    let mut seen = HashSet::new();
    let mut budget = 16;
    crate::ui::file_picker_relevance::mark_tool_paths_from_text(
    ...
    let view = crate::ui::file_picker::FilePickerView::new_with_relevance(root, relevance);
...
fn apply_loaded_session_resets_unpersisted_telemetry() {
    ...
    app.push_turn_cache_record(crate::ui::app::TurnCacheRecord {
        input_tokens: 120,
        output_tokens: 35,
...
async fn apply_loaded_session_resets_workspace_runtime_state() {
    ...
    app.file_tree = Some(crate::ui::file_tree::FileTreeState::new(
...
fn active_tool_status_label_summarizes_live_tool_group() {
    ...
    assert!(label.contains(crate::ui::key_shortcuts::tool_details_shortcut_label()));
}
...
fn fixed_model_auto_thinking_skips_auto_model_router() {
    ...
    assert!(
        !crate::ui::auto_router::should_resolve_auto_model_selection(&app),
...
fn make_agent(
    id: &str,
    status: crate::tools::agent::AgentStatus,
) -> crate::tools::agent::AgentResult {
    crate::tools::agent::AgentResult {
        name: id.to_string(),
        agent_id: id.to_string(),
        context_mode: "fresh".to_string(),
        fork_context: false,
        agent_type: crate::tools::agent::AgentRole::General,
        assignment: crate::tools::agent::AgentAssignment {
...
fn sort_agents_orders_running_before_terminal_statuses() {
    let mut agents = vec![
        make_agent("agent_c", crate::tools::agent::AgentStatus::Completed),
        make_agent("agent_a", crate::tools::agent::AgentStatus::Running),
        make_agent(
            "agent_b",
            crate::tools::agent::AgentStatus::Failed("boom".to_string()),
        ),
    ];
...
fn running_agent_count_unions_cache_and_progress() {
    ...
    app.agent_cache = vec![
        make_agent("agent_a", crate::tools::agent::AgentStatus::Running),
        make_agent("agent_b", crate::tools::agent::AgentStatus::Completed),
    ];
    app.agent_progress
...
fn subagent_token_usage_updates_live_cost_counter_without_card_change() {
    ...
    handle_agent_mailbox(
        &mut app,
        1,
        &crate::tools::agent::MailboxMessage::TokenUsage {
...
fn subagent_token_usage_is_deduped_by_mailbox_sequence() {
    let mut app = create_test_app();
    let usage = crate::tools::agent::MailboxMessage::TokenUsage {
        agent_id: "agent-a".to_string(),
        model: "deepseek-v4-flash".to_string(),
        usage: crate::models::Usage {
            input_tokens: 10_000,
...
fn footer_status_line_spans_show_mode_and_model_idle_and_active() {
    ...
    let _ = app.set_mode(crate::ui::app::AppMode::Limited);
...
fn test_esc_discards_queued_draft_before_clearing_input() {
    ...
    app.is_loading = false;
    app.input.clear();
    app.queued_draft = Some(crate::ui::app::QueuedMessage::new(
...
fn workspace_context_refresh_is_deferred_while_ui_is_busy() {
    ...
    let now = Instant::now();
    crate::ui::workspace_context::refresh_if_needed(&mut app, now, false);

    assert!(app.workspace_context.is_none());
    assert!(app.workspace_context_refreshed_at.is_none());

    crate::ui::workspace_context::refresh_if_needed(&mut app, now, true);
...
fn workspace_context_refresh_respects_ttl_before_requerying_git() {
    ...
    crate::ui::workspace_context::refresh_if_needed(&mut app, start, true);
    let initial = app
        .workspace_context
        .clone()
    ...
    let before_ttl = start + Duration::from_secs(crate::ui::workspace_context::REFRESH_SECS - 1);
    crate::ui::workspace_context::refresh_if_needed(&mut app, before_ttl, true);
    assert_eq!(app.workspace_context.as_deref(), Some(initial.as_str()));

    let after_ttl = start + Duration::from_secs(crate::ui::workspace_context::REFRESH_SECS);
    crate::ui::workspace_context::refresh_if_needed(&mut app, after_ttl, true);
    let refreshed = app
        .workspace_context
        .as_deref()
...
async fn dismissed_plan_prompt_leaves_non_numeric_input_for_normal_send_path() {
    ...
    assert_eq!(
        app.queued_messages
            .front()
            .map(crate::ui::app::QueuedMessage::content),
        Some("yolo".to_string())
    );
    assert_eq!(
...
fn api_key_validation_warns_without_blocking_unusual_formats() {
    assert!(matches!(
        crate::ui::onboarding::validate_api_key_for_onboarding(""),
        crate::ui::onboarding::ApiKeyValidation::Reject(_)
    ));
    assert!(matches!(
        crate::ui::onboarding::validate_api_key_for_onboarding("sk short"),
        crate::ui::onboarding::ApiKeyValidation::Reject(_)
    ));
    assert!(matches!(
        crate::ui::onboarding::validate_api_key_for_onboarding("short-key"),
        crate::ui::onboarding::ApiKeyValidation::Accept { warning: Some(_) }
    ));
    assert!(matches!(
        crate::ui::onboarding::validate_api_key_for_onboarding("averylongkeywithoutdash123456"),
        crate::ui::onboarding::ApiKeyValidation::Accept { warning: Some(_) }
    ));
    assert!(matches!(
        crate::ui::onboarding::validate_api_key_for_onboarding("sk-valid-format-1234567890"),
        crate::ui::onboarding::ApiKeyValidation::Accept { warning: None }
    ));
}
...
fn onboarding_after_api_key_save_does_not_repeat_language_step() {
    ...
    crate::ui::onboarding::advance_onboarding_after_language(&mut app);
...
fn api_key_paste_shortcut_is_not_plain_text_input() {
    ...
    assert!(crate::ui::key_shortcuts::is_paste_shortcut(&ctrl_v));
    assert!(!crate::ui::key_shortcuts::is_text_input_key(&ctrl_v));

    let legacy_ctrl_v = KeyEvent::new(KeyCode::Char('\u{16}'), KeyModifiers::NONE);
    assert!(crate::ui::key_shortcuts::is_paste_shortcut(&legacy_ctrl_v));
    assert!(!crate::ui::key_shortcuts::is_text_input_key(
        &legacy_ctrl_v
    ));

    let shifted = KeyEvent::new(KeyCode::Char('A'), KeyModifiers::SHIFT);
    assert!(crate::ui::key_shortcuts::is_text_input_key(&shifted));
}
...
fn detail_target_prefers_visible_tool_card() {
    ...
    let expected = format!(
        ...
        crate::ui::key_shortcuts::activity_shortcut_label(),
...
fn macos_option_v_glyph_is_treated_as_details_shortcut_only_on_macos() {
    ...
    assert!(crate::ui::key_shortcuts::is_macos_option_v_legacy_key_for_platform(&option_v, true));
    assert!(
        !crate::ui::key_shortcuts::is_macos_option_v_legacy_key_for_platform(&option_v, false)
    );

    let modified = KeyEvent::new(KeyCode::Char('\u{221A}'), KeyModifiers::SHIFT);
    assert!(!crate::ui::key_shortcuts::is_macos_option_v_legacy_key_for_platform(&modified, true));

    let plain_v = KeyEvent::new(KeyCode::Char('v'), KeyModifiers::NONE);
    assert!(!crate::ui::key_shortcuts::is_macos_option_v_legacy_key_for_platform(&plain_v, true));
}
...
fn alt_nav_modifiers_require_alt_and_exclude_ctrl_super() {
    ...
    assert!(!crate::ui::key_shortcuts::alt_nav_modifiers(
        KeyModifiers::NONE
    ));
    assert!(!crate::ui::key_shortcuts::alt_nav_modifiers(
        KeyModifiers::SHIFT
...
fn ctrl_h_is_treated_as_terminal_backspace() {
    assert!(crate::ui::key_shortcuts::is_ctrl_h_backspace(
...
// `running_status_label_with_elapsed` lives in `crate::ui::history` next to
...
fn add_message_does_not_scroll_when_user_scrolled_away() {
    use crate::ui::scrolling::TranscriptScroll;
...
fn recoverable_engine_error_does_not_enter_offline_mode() {
    ...
    assert!(
        matches!(last, crate::ui::history::HistoryCell::Error { .. }),
...
fn env_only_auth_failure_reopens_api_key_onboarding() {
    ...
    app.onboarding = crate::ui::app::OnboardingState::None;
    app.onboarding_needs_api_key = false;
    ...
    assert_eq!(
        app.onboarding,
        crate::ui::app::OnboardingState::ApiKey,
...
fn checklist_write_renders_dedicated_card() {
    ...
    let lines = cell.lines_with_mode(80, true, crate::ui::history::RenderMode::Live);
...
fn notification_settings_tui_always_keeps_configured_method_no_threshold() {
    ...
    let (method, threshold, include_summary) =
        crate::ui::notifications::settings(&config).expect("notification should be enabled");
    assert_eq!(method, crate::ui::notifications::Method::Bel);
...
fn notification_settings_tui_never_disables_notifications() {
    ...
    assert!(crate::ui::notifications::settings(&config).is_none());
}
...
fn notification_settings_no_tui_override_uses_notifications_block() {
    ...
    let (method, threshold, include_summary) =
        crate::ui::notifications::settings(&config).expect("notification should be enabled");
    assert_eq!(method, crate::ui::notifications::Method::Osc9);
...
fn completed_turn_notification_uses_streaming_text() {
    let app = create_test_app();
    let msg = crate::ui::notifications::completed_turn_message(
        &app,
        "Hello there.\n\nWhat's next?",
        false,
