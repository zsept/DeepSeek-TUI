...
use crate::palette;
use crate::ui::app::{App, AppMode, ComposerDensity, VimMode};
...
use crate::ui::scrolling::TranscriptLineMeta;
use crate::{
    commands,
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if has_panel {
            ...
            let hint_line = if self.app.is_history_search_active() {
                ...
                use crate::ui::app::SubmitDisposition;
            ...
            let mode_color = match self.app.mode {
                crate::ui::app::AppMode::Limited => self.app.theme.mode_agent,
                crate::ui::app::AppMode::Yolo => self.app.theme.mode_yolo,
                crate::ui::app::AppMode::Plan => self.app.theme.mode_plan,
            };
        ...
        match (risk, pending) {
            ...
            (RiskLevel::Destructive, Some(opt)) => {
                let again_key = match opt {
                    crate::ui::approval::ApprovalOption::ApproveOnce => confirm_key_once(locale),
                    crate::ui::approval::ApprovalOption::ApproveAlways => {
...
struct ApprovalOptionRow {
    option: crate::ui::approval::ApprovalOption,
    label: &'static str,
    key_hint: &'static str,
    dangerous: bool,
}
...
fn approval_options_for(risk: RiskLevel, locale: Locale) -> [ApprovalOptionRow; 4] {
    use crate::ui::approval::ApprovalOption as O;
...
mod tests {
    ...
    use crate::localization::Locale;
    use crate::palette;
    use crate::ui::app::{App, ComposerDensity, TuiOptions};
    use crate::ui::history::{GenericToolCell, HistoryCell, ToolCell, ToolStatus};
    ...
    fn empty_state_renders_only_without_transcript_activity() {
        ...
        app.add_message(crate::ui::history::HistoryCell::User {
    ...
    fn chat_widget_uses_light_theme_scroll_chrome() {
        let mut app = create_test_app();
        app.theme = crate::ui::theme::LIGHT_THEME;
        app.use_mouse_capture = true;
        for i in 0..120 {
            app.add_message(HistoryCell::User {
                content: format!("user message {i}"),
        ...
        for y in 0..area.height {
            ...
            match cell.symbol() {
                "│" => {
                    saw_track = true;
                    assert_eq!(cell.fg, crate::ui::theme::LIGHT_THEME.border_color);
                }
                "┃" => {
                    saw_thumb = true;
                    assert_eq!(cell.fg, crate::ui::theme::LIGHT_THEME.status_working);
                }
                _ => {}
            }
        }
        ...
        assert_eq!(
            ...
            crate::ui::theme::LIGHT_THEME.status_working
        );
    }
    ...
    fn approval_takeover_clamps_to_short_terminal_height() {
        let request = crate::ui::approval::ApprovalRequest::new(
            "approval-1",
            "exec_shell",
            "Run git commit",
        ...
        let view = crate::ui::approval::ApprovalView::new(request.clone());
    ...
    fn bench_transcript_scroll_5000_messages() {
        ...
        for offset_from_tail in [0usize, 100, 500, 2000] {
            ...
            app.viewport.transcript_scroll =
                crate::ui::scrolling::TranscriptScroll::at_line(target);
