...
pub struct FooterProps {
    ...
    pub retry: deepseek_tui::retry_status::RetryState,
...
impl FooterProps {
    ...
    pub fn from_app(
        ...
        Self {
            ...
            retry: deepseek_tui::retry_status::snapshot(),
        }
    }
}
...
fn retry_banner_spans(max_width: usize, props: &FooterProps) -> Option<Vec<Span<'static>>> {
    let (label, color) = match &props.retry {
        deepseek_tui::retry_status::RetryState::Active(banner) => {
            ...
            (
                ...
                deepseek_tui::palette::STATUS_WARNING,
            )
        }
        deepseek_tui::retry_status::RetryState::Failed { reason, .. } => {
            (format!("× failed: {reason}"), deepseek_tui::palette::STATUS_ERROR)
        }
        deepseek_tui::retry_status::RetryState::Idle => return None,
    };
...
mod tests {
    ...
    fn idle_props_for(app: &App) -> FooterProps {
        ...
        props.retry = deepseek_tui::retry_status::RetryState::Idle;
        props
    }
    ...
    fn render_shows_retry_banner_when_active() {
        ...
        props.retry = deepseek_tui::retry_status::RetryState::Active(deepseek_tui::retry_status::RetryBanner {
            attempt: 2,
    ...
    fn render_shows_failure_row_when_failed() {
        let app = make_app();
        let mut props = idle_props_for(&app);
        props.retry = deepseek_tui::retry_status::RetryState::Failed {
